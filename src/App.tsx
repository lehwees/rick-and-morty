...
export default function App() {
  ...
  const [favorites, setFavorites] = useState<number[]>(() => {
    const saved = localStorage.getItem("rick_and_morty_favorites");
    return saved ? JSON.parse(saved) : [];
  });
  const [showFavoritesOnly, setShowFavoritesOnly] = useState(false);

  useEffect(() => {
    localStorage.setItem("rick_and_morty_favorites", JSON.stringify(favorites));
  }, [favorites]);
  ...
  const characters = showFavoritesOnly
    ? allCharacters.filter((c) => favorites.includes(c.id))
    : allCharacters;
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gray-50 text-gray-900 dark:bg-gray-950 dark:text-white transition-colors duration-300 p-8">
        ...
        {isError && !isLoading && !showFavoritesOnly && (
          <div className="flex h-[60vh] flex-col items-center justify-center text-center p-4">
            ...
            <h2 className="text-2xl font-bold text-red-500 dark:text-red-400 mb-2">
              Houve um problema no multiverso
            </h2>
            <p className="text-gray-600 dark:text-gray-400 max-w-sm mb-6">
              Não conseguimos sintonizar a API do Rick and Morty. Verifique sua
              conexão.
            </p>
            <button
        ...
        {(!isLoading && !isError) || showFavoritesOnly ? (
          <>
            <header className="mb-8 flex flex-col items-center justify-center">
              ...
              />
              <p className="text-gray-600 dark:text-gray-400 mt-4 p-6 text-lg font-medium">
                Explore os personagens do multiverso
              </p>
            </header>
            <section className="max-w-[1920px] mx-auto mb-12 flex flex-col md:flex-row gap-4 justify-center items-center">
              <div className="flex gap-2 w-full sm:max-w-md">
                <input
                  ...
                  onKeyDown={(e) =>
                    e.key === "Enter" && setSearchName(inputValue)
                  }
              ...
              <div className="relative w-full sm:w-52">
                <select
                  ...
                  className="w-full px-5 pr-14 py-3 rounded-xl bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-800 text-gray-900 dark:text-white focus:outline-none focus:border-green-500 transition-colors font-medium cursor-pointer shadow-sm appearance-none"
                >
                ...
                <div className="pointer-events-none absolute inset-y-0 right-5 flex items-center text-gray-500 dark:text-gray-400">
                  <svg
                    className="w-4 h-4"
                    fill="none"
                    stroke="currentColor"
              ...
              <button
                ...
                className={`w-full sm:w-auto px-5 py-3 rounded-xl font-medium transition-colors cursor-pointer shadow-sm flex items-center justify-center gap-2 border ${
                  showFavoritesOnly
                    ? "bg-yellow-500 text-gray-950 border-yellow-600 shadow-md font-bold"
                    : "bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-800 hover:border-yellow-500"
                }`}
              >
                <span>⭐</span>{" "}
            ...
            {characters.length === 0 ? (
              <div className="text-center mt-12 text-gray-500 dark:text-gray-400 text-xl font-medium flex flex-col items-center justify-center gap-2">
                <p>
                  ...
                  : "Nenhum habitante encontrado nesta dimensão... 🛸"}
              </p>
              <button
                onClick={handleLogoClick}
          ...
          {!showFavoritesOnly && (
            <div
              ref={ref}
              className="w-full flex flex-col items-center justify-center h-20 mt-8 mb-12"
            >
              {isFetchingNextPage && (
              ...
              {!hasNextPage && characters.length > 0 && (
                <p className="text-gray-500 font-medium">
                  Você chegou ao fim do multiverso!
                </p>
              )}
            </div>
          )}
      ...
      <Modal
        ...
        isFavorite={
          selectedCharacter ? favorites.includes(selectedCharacter.id) : false
        }
        onToggleFavorite={toggleFavorite}
      />
    </div>
